...
import com.example.balcamgym.DTO.ProductStorageDTO;
...
public class PdfGenerator {
    ...
    public static void createBill (ProductStorageBillDTO productStorageBillDTO, BillDTO billDTO){
        ...
        try {
            ...
            PdfWriter.getInstance(document, new FileOutputStream(route + "/download/BALCAM_BILL"));
            ...
            Set<Long> ids = new HashSet<>(productStorageBillDTO.getIds());




            ids.forEach(product -> {
                int quantity = Collections.frequency(productStorageBillDTO.getIds().stream().map(product1 -> product1.getName()).collect(Collectors.toList()), product.getName());
                PdfPCell pdfPCell6 = new PdfPCell(new Paragraph(product., subFont));
